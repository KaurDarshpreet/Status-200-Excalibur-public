...
const IssueCard = ({ name, index, handleOnClick }: IssueCardProps) => {
    return (
        <div className={`border:none mt-1 text-slate-700 font-semibold rounded bg-gradient-to-r from-[#00FFF5] to-[#00ADB5] text-lg  transition-all shadow-[0_0_1px_#00FFF5] hover:shadow-none text-center py-2 px-4 cursor-pointer`} onClick={() => handleOnClick(index)}>
...
export default function ReviewPage({ issues }: ReviewPageProps) {
    const [notReviewed, setNotReviewed] = useState(issues.filter(issue => issue.is_resolved));
    useEffect(() => {
    ...
    if(notReviewed.length == 0){
        return (<div className="bg-slate-200 min-h-[94svh] min-w-[73svw] flex flex-col items-center justify-evenly rounded-lg font-bold text-5xl font-sans">ALL ISSUES REVIEWED 🎊</div>)
    }
    ...
    const handleReview = (index: number)=>{
        ...
        reviewStudentIssue.mutate(issue_id);
        setIdx(0);
    }
    return (
        <>
            <div className="flex flex-col text-white font-semibold h-[94svh] overflow-auto basis-[100%] p-2 gap-1">
                {issues.map((issue, index) => {
                    return (
                            (issue.is_resolved && (issue.technician != null) && <IssueCard key={index} name={issue.title} handleOnClick={handleOnClick} index={index} />)
                    )
                })}
            </div>
            {issues[idx].is_resolved && (issues[idx].technician != null) && <div className="bg-[#393E46] p-[5%] min-h-[94svh] min-w-[40svw] flex flex-col justify-evenly rounded-lg">
                ...
                {issues[idx].issue_media && (
                    <div className="min-w-[30svw] min-h-[50svh] bg-[#222831] m-auto rounded-lg p-1 flex items-center justify-center">
                        {issues[idx].issue_media.endsWith(".jpg") || issues[idx].issue_media.endsWith(".jpeg") ||
                            issues[idx].issue_media.endsWith(".png") || issues[idx].issue_media.endsWith(".gif") ? <img src={issues[idx].issue_media} alt="Issue Media" className="w-full h-full object-cover" /> : <video src={issues[idx].issue_media} controls className="w-full h-full object-cover" />}
                    </div>
                )}
