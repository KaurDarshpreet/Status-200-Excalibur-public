...
export default function StudentProfile() {
    ...
    return (
        <div className="profile flex flex-col justify-center gap-2 items-center bg-[#393E46] w-[20svw] mx-auto mt-5 rounded-lg shadow-slate-700 shadow-sm p-2 grow">
            ...
            <br />
            <div className="profile_email text-slate-700 font-bold py-1 px-2  bg-gradient-to-r from-[#00FFF5] to-[#00ADB5]  rounded-lg text-2xl">{user.rollNo}</div>
            <div className="profile_email text-white text-lg font-semibold my-1">{user.email}</div>
            <div className="profile_email text-white text-2xl font-bold mb-2 flex items-center gap-3">{user.hostel} <span className='text-2xl bg-[#00FFF5] text-slate-700 px-1 rounded-md py-0'>{user.roomNo}</span></div>
        </div>
    )
}